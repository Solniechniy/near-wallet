...
export const calculateAPY = async (validator, tokenPrices) => {
    ...
    try {
        ...
        const totalStakedBalance = big(validator?.poolSummary.total_staked_balance);

        const summaryAPY = activeFarms.reduce((acc, farm) => {
            const tokenPriceInUSD = big(+tokenPrices[farm.token_id].price);
            const nearPriceInUSD = big(+tokenPrices[NEAR_TOKEN_ID].price);

            const rewardsPerSecond = big(farm.amount).div(big((farm.end_date - farm.start_date)* 1e9));
            const rewardsPerSecondInUSD = rewardsPerSecond.mul(tokenPriceInUSD);
            const totalStakedBalanceInUSD = totalStakedBalance.mul(nearPriceInUSD);
            const farmAPY = rewardsPerSecondInUSD.mul(SECONDS_IN_YEAR).div(totalStakedBalanceInUSD).mul(100);
            return acc.add(farmAPY);
        }, big(0));

        return summaryAPY.round(3).toFixed(2);
    }
    catch (e) {
